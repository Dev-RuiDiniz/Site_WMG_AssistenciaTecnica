    ...
    <form className="mt-8 grid gap-6" noValidate onSubmit={handleSubmit}>
      <div className="grid gap-5 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-bold text-white">
          Nome *
          <input className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.name} onChange={(event) => update('name', event.target.value)} />
          <ErrorText message={errors.name} />
        </label>

        <label className="grid gap-2 text-sm font-bold text-white">
          Empresa
          <input className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.company} onChange={(event) => update('company', event.target.value)} />
        </label>

        <label className="grid gap-2 text-sm font-bold text-white">
          E-mail
          <input className="rounded-2xl px-4 py-3 text-wmg-navy-950" type="email" value={values.email} onChange={(event) => update('email', event.target.value)} />
          <ErrorText message={errors.email} />
        </label>

        <label className="grid gap-2 text-sm font-bold text-white">
          Telefone / WhatsApp
          <input className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.phone} onChange={(event) => update('phone', event.target.value)} />
          <ErrorText message={errors.phone} />
        </label>

        <label className="grid gap-2 text-sm font-bold text-white">
          Equipamento *
          <select className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.equipment} onChange={(event) => update('equipment', event.target.value)}>
        ...
        <label className="grid gap-2 text-sm font-bold text-white">
          Tipo de falha *
          <select className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.failureType} onChange={(event) => update('failureType', event.target.value)}>
        ...
        <label className="grid gap-2 text-sm font-bold text-white md:col-span-2">
          Urgência *
          <select className="rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.urgency} onChange={(event) => update('urgency', event.target.value)}>
        ...
        <label className="grid gap-2 text-sm font-bold text-white md:col-span-2">
          ...
          <textarea className="min-h-36 rounded-2xl px-4 py-3 text-wmg-navy-950" value={values.description} onChange={(event) => update('description', event.target.value)} />
      ...
      <div className="flex flex-wrap gap-4">
        <button type="submit" className="rounded-full bg-wmg-lime-500 px-6 py-3 font-extrabold uppercase text-wmg-navy-950">
          Enviar diagnóstico
        </button>
      ...
      <div role="status" aria-live="polite">
        {status === 'success' ? <p className="rounded-2xl bg-wmg-lime-500/20 p-4 font-semibold text-wmg-lime-500">{contactContent.successMessage}</p> : null}
        {status === 'error' ? <p className="rounded-2xl bg-red-500/20 p-4 font-semibold text-red-100">{contactContent.errorMessage}</p> : null}
      </div>
    </form>
  );
}
